# =============================================================================
# [EN] 07a_lasso_GLM.R -- LASSO variable selection (GLM binomial) with unpenalized latent factors and bootstrap stability
# INPUTS:  rdos/datos/06_theta_predichos.rds, rdos/modelos/06_modelo_heterofactor.rds
# OUTPUTS: rdos/modelos/07_modelo_lasso_GLM*.rds, rdos/contratos/07a_contrato_lasso_GLM*.rds
# =============================================================================
# 🌟 07a_lasso_GLM.R 🌟 ####
# OBJETIVO:
#    Selección de variables mediante LASSO (GLM binomial) para predecir
...
#
# OUTPUTS:
#    - PATH_07_MODELO_GLM    → rdos/modelos/07_modelo_lasso_GLM3T.rds
#    - PATH_07_RECIPE_GLM    → rdos/modelos/07_recipe_lasso_GLM3T.rds
#    - 07a_vars_sel_GLM3T.csv → rdos/reportes/
#    - PATH_07_CONTRATO_GLM  → rdos/contratos/07a_contrato_lasso_GLM3T.rds
#
...
#    - family = "binomial" (GLM logístico), type.measure = "auc"
#    - Predicciones naturalmente en [0,1] — sin necesidad de clipear
#
# REFERENCIA LPM (para comparar):
#    AUC test: 0.8627 | Vars sel (λ.1se): 81 | % fuera [0,1]: 6.18%
#
...
cat("═══════════════════════════════════════════════════════════════════\n")
cat("  🚀 SCRIPT 07a — LASSO GLM\n")
cat(sprintf("  Sufijo de modelo: %s | N_TRIMESTRES_TRAINING: %d\n",
            SUFIJO_MODELO_GLM, N_TRIMESTRES_TRAINING))
...
# 🪫 1. Carga del panel y extracción de θ del Modelo A -------------------------
...
# 🪫 2. Preparación del universo de entrenamiento ------------------------------
...
recipe_lasso_glm <- recipe(formalidad_bin ~ ., data = df_recipe) %>%
...
tic("prep recipe")
recipe_prepped <- prep(recipe_lasso_glm, training = df_recipe)
toc()
...
cat("   Cluster activado. Corriendo cv.glmnet (binomial)...\n")
...
cat(sprintf("   λ.min:  %.6f | AUC CV: %.4f\n",
            lambda_min, max(cv_fit$cvm)))
cat(sprintf("   λ.1se:  %.6f | AUC CV: %.4f\n",
...
# GLM binomial: predict con type = "response" entrega probabilidades en [0,1]
pred_raw  <- as.vector(predict(cv_fit, newx = X_test, s = "lambda.1se",
                               type = "response"))
pred_clip <- pmax(0, pmin(1, pred_raw))   # por construcción debería ser innecesario
pct_fuera <- mean(pred_raw < 0 | pred_raw > 1) * 100

cat(sprintf("   Predicciones fuera de [0,1]: %.2f%%\n", pct_fuera))
if (pct_fuera > 0.1) {
  warning(sprintf("⚠️  %.2f%% predicciones fuera de [0,1] en GLM binomial. Revisar.", pct_fuera))
} else {
  cat("   [✅] Predicciones acotadas en [0,1] por construcción (GLM binomial)\n")
}

# Referencia LPM: 6.18% fuera de [0,1] → GLM debería ser ~0%
cat(sprintf("   [REF LPM: 6.18%% | GLM: %.2f%%]\n", pct_fuera))
...
cat(sprintf("   [REF LPM AUC: 0.8627]\n"))
...
boot_results <- foreach(
  ...
  # GLM binomial: predict type = "response" ya en [0,1]
  pred_b <- as.vector(predict(fit_b, newx = X_test, s = lambda_1se,
                              type = "response"))
  auc_b  <- tryCatch(
...
# Matriz completa de coeficientes bootstrap (para bandas de incertidumbre A2)
path_coef_boot <- file.path(DIR_MODELOS, paste0("07a_coef_boot_", SUFIJO_MODELO_GLM, ".rds"))
saveRDS(coef_boot, path_coef_boot)
cat(sprintf("   [✅] %s (%d draws × %d coefs)\n",
            basename(path_coef_boot), nrow(coef_boot), ncol(coef_boot)))

saveRDS(cv_fit,        PATH_07_MODELO_GLM)
cat(sprintf("   [✅] %s\n", basename(PATH_07_MODELO_GLM)))

saveRDS(recipe_prepped, PATH_07_RECIPE_GLM)
cat(sprintf("   [✅] %s\n", basename(PATH_07_RECIPE_GLM)))

path_vars_csv <- file.path(DIR_REPORTES, paste0("07a_vars_sel_", SUFIJO_MODELO_GLM, ".csv"))
...
saveRDS(contrato_07a, PATH_07_CONTRATO_GLM)
cat(sprintf("   [✅] %s\n", basename(PATH_07_CONTRATO_GLM)))
...
cat("\n═══════════════════════════════════════════════════════════════════\n")
cat("  📋 RESUMEN SCRIPT 07a — LASSO GLM\n")
cat("═══════════════════════════════════════════════════════════════════\n")
...
toc()
cat("\n✅ Script 07a GLM completado exitosamente\n")
