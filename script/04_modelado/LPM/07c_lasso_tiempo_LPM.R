# =============================================================================
# [EN] 07c_lasso_tiempo_LPM.R -- Temporal neutrality test: verify time effects are absorbed by structural covariates (LPM)
# INPUTS:  rdos/datos/06_theta_predichos.rds, contract from 07a LPM
# OUTPUTS: rdos/contratos/07c_contrato_tiempo_LPM*.rds
# =============================================================================
# 🌟 07c_lasso_tiempo_LPM.R 🌟 ####
# CAPA 4 — MODELADO LPM | Test de neutralidad temporal
#
...
#
# INPUT:    PATH_06_THETA, PATH_06_MODELO_HETERO, PATH_07_CONTRATO (n_train/n_test)
# OUTPUTS:  PATH_07C_CONTRATO (.rds)
#           rdos/reportes/07c_tabla_temporal_LPM3T.csv
#
# LECCIONES APLICADAS:
...
tic("Script 07c completo")
start_time <- Sys.time()

cat("═══════════════════════════════════════════════════════════════════\n")
cat("  🌟 SCRIPT 07c — TEST DE NEUTRALIDAD TEMPORAL (LPM)\n")
...
# Cargar contrato_07a para verificación del split
contrato_07a <- readRDS(PATH_07_CONTRATO)
cat("  Contrato 07a cargado | n_train:", format(contrato_07a$n_train, big.mark = ","),
...
# VARS_BASE: idéntico al modelo 07a/07b
VARS_BASE <- c(
...
# 🪫 3. Recipe (base 07a + vars temporales) ------------------------------------
cat("\n── FASE 3: Recipe ──────────────────────────────────────────────────\n")

# NOTA: se construye un recipe propio (no se reutiliza PATH_07_RECIPE_LASSO)
...
# Comparación con referencia 07b (OLS post-LASSO — modelo principal)
# HC documentado: fallback tryCatch — fuente primaria es PATH_07B_CONTRATO$resumen_comparacion$auc_roc
.AUC_BASE_FB_07C <- 0.8659
tryCatch({
  contrato_07b <- readRDS(PATH_07B_CONTRATO)
  rc           <- contrato_07b$resumen_comparacion
  auc_base_07b <- rc$auc_roc[1]
  cat("  Contrato 07b LPM: AUC base =", round(auc_base_07b, 4), "\n")
}, error = function(e) {
  warning(sprintf("[07c-LPM] No se pudo leer AUC de contrato 07b — usando HC fallback. Error: %s", e$message))
...
# Guardar CSV — lección 27: col.names = FALSE si se escribe encabezado manual
path_csv_07c <- file.path(DIR_REPORTES,
                          paste0("07c_tabla_temporal_", SUFIJO_MODELO_LPM, ".csv"))
write_csv(tabla_temporal_07c, path_csv_07c)
cat("  [✅] 07c_tabla_temporal_", SUFIJO_MODELO_LPM, ".csv guardado\n",
    sep = "")
...
contrato_07c <- list(
  # ── Identificación ───────────────────────────────────────────────────────
  script      = "07c_lasso_tiempo_LPM.R",
  sufijo      = SUFIJO_MODELO_LPM,
  fecha       = Sys.time(),
  version_tag = "v1_neutralidad_temporal_LPM",
...
saveRDS(contrato_07c, PATH_07C_CONTRATO)
cat("  [✅] Contrato guardado:", basename(PATH_07C_CONTRATO), "\n")
...
cat("\n═══════════════════════════════════════════════════════════════════\n")
cat("  📑 RESUMEN SCRIPT 07c — TEST TEMPORAL LPM\n")
cat("═══════════════════════════════════════════════════════════════════\n")
