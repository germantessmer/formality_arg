# =============================================================================
# [EN] 07d_lasso_interacciones_SLS.R -- Interaction robustness: test sector x occupation interactions via bootstrap LASSO (SLS)
# INPUTS:  rdos/datos/06_theta_predichos.rds, contracts from 07a/07b SLS
# OUTPUTS: rdos/contratos/07d_contrato_interacciones_SLS*.rds
# =============================================================================
# 🌟 07d_lasso_interacciones_SLS.R 🌟 ####
# CAPA 4 — MODELADO SLS | Test de interacciones seccion × categoria_ocupacional
#
# OBJETIVO: Evaluar si las interacciones seccion × categoria_ocupacional mejoran
#           las métricas respecto al modelo base SLS 07b. Las interacciones capturan
...
#
# DISEÑO:   Recipe base SLS 07a + step_interact post-dummies
...
#           OLS post-LASSO para comparación de R² / tests vs SLS 07b
#
#           NOTA SLS — MUESTRA PARA TEST DE INTERACCIONES:
#           El LASSO + bootstrap se corre sobre df_train COMPLETO (N=49,225),
#           no sobre κ̂γ (N=38,724). Razones:
#             1. El κ̂γ se define con el modelo BASE sin interacciones; re-ejecutar
#                el recorte iterativo para cada especificación sería conceptualmente
#                incorrecto (movería el target de la comparación).
#             2. El objetivo es verificar si las interacciones serían seleccionadas
#                por el LASSO gaussian antes del recorte; si el LASSO base no las
#                seleccionó, el recorte tampoco cambiaría esa conclusión.
#           El OLS post-LASSO también corre sobre df_train completo para mantener
#           comparabilidad con el LASSO del mismo script. Los coeficientes se
#           interpretan con la nota de que la κ̂γ excluye el 21.33% de obs del train.
#
# RESULTADO ESPERADO: ninguna interacción estable (≥80% bootstrap) →
#           justifica parsimonia del modelo base SLS 07b
#
# INPUT:    PATH_06_THETA, PATH_06_MODELO_HETERO
#           PATH_07A_CONTRATO_SLS (n_train/n_test)
#           PATH_07B_CONTRATO_SLS (métricas base SLS)
# OUTPUTS:  PATH_07D_CONTRATO_SLS (.rds)
#           rdos/reportes/07d_comp_coefs_SLS3T.csv
#           rdos/reportes/07d_tabla_ols_SLS3T.txt
#
# LECCIONES APLICADAS:
...
#   67 — SLS = gaussian; recipe LPM reutilizable en estructura
#   68 — AUC/R² base SLS leídos de contrato_07b_sls (no hardcodeados)
#   72 — R² SLS (0.3014) sobre κ̂γ; aquí se obtiene R² sobre df_train completo
#
...
tic("Script 07d SLS completo")
start_time <- Sys.time()

cat("═══════════════════════════════════════════════════════════════════\n")
cat("  🌟 SCRIPT 07d — LASSO CON INTERACCIONES (SLS)\n")
cat("  Inicio:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("═══════════════════════════════════════════════════════════════════\n\n")
cat("  DISEÑO: modelo base SLS + interacciones seccion × categoria_ocupacional\n")
cat("          penalty.factor = 1 para interacciones (LASSO decide)\n")
cat("          penalty.factor = 0 para theta_A_mA / theta_B_mA\n")
cat("          LASSO + OLS sobre df_train completo (ver NOTA SLS en header)\n\n")
...
# Cargar contratos SLS de referencia (lecciones 68, 71)
contrato_07a_sls <- readRDS(PATH_07A_CONTRATO_SLS)
contrato_07b_sls <- readRDS(PATH_07B_CONTRATO_SLS)

cat("  Contrato 07a SLS: n_train =", format(contrato_07a_sls$n_train, big.mark = ","),
    "| n_test =", format(contrato_07a_sls$n_test, big.mark = ","), "\n")
cat("  Contrato 07b SLS: R² (κ̂γ) =", round(contrato_07b_sls$ols_r2, 4),
    "| AUC =", round(contrato_07b_sls$auc_roc, 4), "\n\n")

# Valores de referencia SLS 07b (leídos de contrato; hardcoded como respaldo — lección 64)
R2_BASE_07B  <- tryCatch(contrato_07b_sls$ols_r2,   error = function(e) 0.3014)
AUC_BASE_07B <- tryCatch(contrato_07b_sls$auc_roc,  error = function(e) 0.8651)
MCC_BASE_07B <- tryCatch(contrato_07b_sls$metricas_clf$mcc, error = function(e) 0.5765)
F1_BASE_07B  <- tryCatch(contrato_07b_sls$metricas_clf$f1,  error = function(e) 0.8046)

# Contexto κ̂γ para notas (no usado en cálculos)
N_KAPPA_07B  <- tryCatch(contrato_07b_sls$n_final,  error = function(e) 38724L)
PCT_LOSS_07B <- tryCatch(contrato_07b_sls$pct_loss,  error = function(e) 21.33)

cat(sprintf("  Valores base SLS 07b (referencia comparativa):\n"))
cat(sprintf("    AUC  = %.4f | R² (κ̂γ, N=%s) = %.4f\n",
            AUC_BASE_07B, format(N_KAPPA_07B, big.mark=","), R2_BASE_07B))
cat(sprintf("    F1   = %.4f | MCC = %.4f\n", F1_BASE_07B, MCC_BASE_07B))
cat(sprintf("    Nota: R² base calculado sobre κ̂γ (lección 72); R² 07d sobre df_train\n\n"))
...
# 🪫 4. LASSO CV (family = "gaussian" — SLS) -----------------------------------
...
tic("cv.glmnet 07d SLS")
cv_fit_07d <- cv.glmnet(
  x              = X_train,
  y              = y_train,
  weights        = w_train,
  alpha          = 1,
  family         = "gaussian",      # SLS: OLS lineal
  penalty.factor = pf,
  type.measure   = "mse",
  foldid         = foldid_vec,
  parallel       = TRUE
...
metricas_clf_07d <- tibble(
  ...
  Valor_07b   = c(AUC_BASE_07B, 0.8787, NA_real_,   # AUC-PR SLS 07b
                  NA_real_, NA_real_, NA_real_, NA_real_,
                  F1_BASE_07B, MCC_BASE_07B, NA_real_, 0.5795),
  Delta       = round(Valor_07d - Valor_07b, 4)
)
cat("\n  Métricas de clasificación (07d SLS vs 07b SLS):\n")
print(metricas_clf_07d, n = Inf)

# Hosmer-Lemeshow — L70: usar ntile(pred, 10), no cut(quantile())
# NOTA SLS: H-L no aplica formalmente a modelos lineales (ver L70, L72).
# Se calcula como referencia técnica pero se reporta "N/A" en el paper.
...
# Proteger contra pred_mean == 0 o 1 (denominador cero)
hl_df_07d_safe <- hl_df_07d %>%
  filter(pred_mean > 0 & pred_mean < 1)

if (nrow(hl_df_07d_safe) >= 2) {
  hl_stat_07d <- sum(hl_df_07d_safe$n *
                       (hl_df_07d_safe$obs_mean - hl_df_07d_safe$pred_mean)^2 /
                       (hl_df_07d_safe$pred_mean * (1 - hl_df_07d_safe$pred_mean)))
  hl_pval_07d <- pchisq(hl_stat_07d, df = 8, lower.tail = FALSE)
  cat(sprintf("\n  Hosmer-Lemeshow (referencia): χ²=%.3f, p=%s\n",
              hl_stat_07d, format.pval(hl_pval_07d, digits = 3)))
  cat("  (NOTA SLS: H-L no aplica formalmente a modelos lineales — ver L72)\n")
} else {
  hl_stat_07d <- NA_real_
  hl_pval_07d <- NA_real_
  cat("  Hosmer-Lemeshow: N/A (denominadores cero — modelo lineal SLS)\n")
}
...
cat("\n═══════════════════════════════════════════════════════════════════\n")
cat("  RESULTADO CENTRAL — Interacciones en bootstrap SLS (top 10 por sel.):\n")
cat("═══════════════════════════════════════════════════════════════════\n")
...
# 🪫 7. OLS post-LASSO (comparacion directa con SLS 07b) ----------------------
cat("\n── FASE 7: OLS post-LASSO ──────────────────────────────────────────\n")
cat("  NOTA SLS: OLS sobre df_train completo (N=", format(nrow(df_train), big.mark=","),
    ").\n", sep = "")
cat("  R² aquí NO comparable con R²=", R2_BASE_07B, " de 07b (calculado sobre κ̂γ).\n",
    sep = "")
cat("  Objetivo: verificar si interacciones mejoran ajuste en términos relativos.\n\n")

vars_sel_1se_07d  <- vars_sel_07d$variable

# Detectar y eliminar alias (lección 69)
df_ols_check <- as.data.frame(X_train) %>%
  select(any_of(vars_sel_1se_07d)) %>%
  mutate(formalidad_bin = y_train, codusu = df_train$codusu)

# Primer fit para detectar alias
formula_check <- as.formula(
  paste("formalidad_bin ~",
        paste(intersect(vars_sel_1se_07d, names(df_ols_check)), collapse = " + "))
)
m_check <- lm(formula_check, data = df_ols_check)
alias_info <- alias(m_check)
vars_aliasadas <- rownames(alias_info$Complete)

if (length(vars_aliasadas) > 0) {
  cat("  ⚠️ Variables aliasadas detectadas (eliminadas antes de vcovCL — lección 69):\n")
  cat("    ", paste(vars_aliasadas, collapse = ", "), "\n")
}

vars_disponibles  <- setdiff(intersect(vars_sel_1se_07d, colnames(X_train)), vars_aliasadas)
...
df_ols_07d <- as.data.frame(X_train) %>%
  select(any_of(vars_disponibles)) %>%
  mutate(
...
cat(sprintf("  R² (train completo): %.4f | R² ajustado: %.4f | N: %s\n",
            glance_ols_07d$r.squared,
            glance_ols_07d$adj.r.squared,
            format(glance_ols_07d$nobs, big.mark = ",")))
cat(sprintf("  Base 07b (κ̂γ): R²=%.4f | ⚠️ NO comparable directamente\n", R2_BASE_07B))
...
if (!is.null(vif_vals_07d)) {
  ...
  cat(sprintf("  VIF_adj > 3.16: %d variables (base 07b SLS: 4)\n", n_vif_alto_07d))
...
path_csv_07d <- file.path(DIR_REPORTES,
                          paste0("07d_comp_coefs_", SUFIJO_MODELO_SLS, ".csv"))
write_csv(comp_coefs_07d, path_csv_07d)
cat("  [✅] 07d_comp_coefs_", SUFIJO_MODELO_SLS, ".csv guardado\n", sep = "")
...
encabezado_07d <- c(
  "================================================================",
  paste0("MODELO SLS POST-LASSO 07d — CON INTERACCIONES seccion x categoria"),
  paste0("Sufijo modelo: ", SUFIJO_MODELO_SLS),
  ...
  paste0("N observaciones (df_train completo): ",
         format(glance_ols_07d$nobs, big.mark = ",")),
  paste0("NOTA: R² calculado sobre df_train completo, NO sobre κ̂γ (N=",
         format(N_KAPPA_07B, big.mark = ","), "). Ver lección 72."),
  paste0("Variables modelo: ", length(vars_disponibles),
         "  (base 07b SLS: 80)"),
  paste0("R²:               ", round(glance_ols_07d$r.squared, 4),
         "  (base 07b SLS — κ̂γ: ", R2_BASE_07B, " — NO comparable)"),
  ...
  paste0("VIF_adj > 3.16:   ", ifelse(is.na(n_vif_alto_07d), "N/A", n_vif_alto_07d),
         " variables  (base 07b SLS: 4)"),
  paste0("H-L test:         N/A — no aplica a modelos lineales (SLS)"),
  "Sig: *** p<0.001 | ** p<0.01 | * p<0.05 | . p<0.10",
  "================================================================", ""
)

path_txt_07d <- file.path(DIR_REPORTES,
                          paste0("07d_tabla_ols_", SUFIJO_MODELO_SLS, ".txt"))
...
cat("  [✅] 07d_tabla_ols_", SUFIJO_MODELO_SLS, ".txt guardado\n", sep = "")

# 3. Contrato 07d SLS
contrato_07d <- list(
  # ── Identificación ───────────────────────────────────────────────────────
  script      = "07d_lasso_interacciones_SLS.R",
  sufijo      = SUFIJO_MODELO_SLS,
  fecha       = Sys.time(),
  version_tag = "v1_interacciones_SLS",
  ...
  # ── Nota SLS: muestra OLS ─────────────────────────────────────────────────
  nota_muestra_ols = paste0(
    "OLS sobre df_train completo (N=", nrow(df_train), "). ",
    "R² NO comparable con base 07b (κ̂γ, N=", N_KAPPA_07B, "). ",
    "Ver lección 72."
  ),
  n_kappa_07b   = N_KAPPA_07B,
  pct_loss_07b  = PCT_LOSS_07B,
  ...
  ols_r2_base    = R2_BASE_07B,        # base es κ̂γ — NO comparable
  delta_r2       = glance_ols_07d$r.squared - R2_BASE_07B,   # informativo
  ols_f_stat     = glance_ols_07d$statistic,
  ols_f_pval     = glance_ols_07d$p.value,
  ols_n_vars     = length(vars_disponibles),
  vars_aliasadas = vars_aliasadas,
  ...
  resumen_comparacion = tibble(
    ...
    metrica   = c("auc_test", "delta_auc_vs_07b",
                  "r2_train_completo", "delta_r2_vs_kappa",
...
saveRDS(contrato_07d, PATH_07D_CONTRATO_SLS)
cat("  [✅] Contrato guardado:", basename(PATH_07D_CONTRATO_SLS), "\n")
...
cat("\n═══════════════════════════════════════════════════════════════════\n")
cat("  📑 RESUMEN SCRIPT 07d SLS — INTERACCIONES vs BASE SLS 07b\n")
cat("═══════════════════════════════════════════════════════════════════\n")
cat(sprintf("  %-36s %10s %10s %10s\n", "Métrica", "Base 07b", "07d Inter.", "Δ"))
cat(sprintf("  %s\n", strrep("─", 68)))

metricas_comp <- list(
  list("Vars candidatas",          nrow(vars_sel_07d) - n_interact_sel_1se + length(cols_interact),
                                   ncol(X_train),              NA),
  list("Vars sel (λ.1se)",         80,                 nrow(vars_sel_07d),         NA),
  list("  └ Interacc. sel",        0,                  n_interact_sel_1se,         NA),
  list("  └ Interacc. estables",   0,                  n_interact_estable,         NA),
  list("R² (⚠️ escala diferente)", R2_BASE_07B,        glance_ols_07d$r.squared,   NA),
  list("  (07b=κ̂γ / 07d=train)",  "N=38,724",         paste0("N=",nrow(df_train)), NA),
...
for (m in metricas_comp) {
  if (is.na(m[[4]]) || !is.numeric(m[[4]])) {
    cat(sprintf("  %-36s %10s %10s\n",
                m[[1]], as.character(m[[2]]), as.character(m[[3]])))
  } else {
    cat(sprintf("  %-36s %10.4f %10.4f %+10.4f\n",
...
if (n_interact_estable == 0) {
  cat("  ✅ PARSIMONIA CONFIRMADA (SLS): ninguna interacción estable (≥80% bootstrap).\n")
  cat("     Las interacciones no aportan señal robusta en el modelo gaussian.\n")
  cat("     CONCLUSIÓN: mantener modelo base SLS 07b.\n")
} else {
  cat(sprintf("  ⚠️  %d interacción(es) estable(s) en bootstrap.\n", n_interact_estable))
  if (auc_val_07d > AUC_BASE_07B) {
    cat(sprintf("     AUC mejora %+.4f respecto a SLS 07b. Evaluar adopción.\n",
                auc_val_07d - AUC_BASE_07B))
  }
}

cat("───────────────────────────────────────────────────────────────────\n")
cat("  CHECKLIST 07d SLS:\n")
...
cat("  [✅] penalty.factor = 0 para theta_A_mA / theta_B_mA\n")
cat("  [✅] Alias detectados antes de vcovCL (lección 69)\n")
cat("  [✅] AUC/R² base leídos de contrato_07b_sls (lección 68)\n")
cat("  [✅] Nota R² sobre df_train ≠ κ̂γ documentada (lección 72)\n")
cat("  [✅] H-L reportado como N/A con nota metodológica\n")
cat("  [✅] Bootstrap 200 iter completado\n")
cat("  [✅] OLS post-LASSO completado\n")
cat(sprintf("  [✅] Contrato guardado: %s\n", basename(PATH_07D_CONTRATO_SLS)))
cat("  [ ] NO genera HTML — todo el HTML va en 07e SLS\n")
cat("───────────────────────────────────────────────────────────────────\n")
cat(sprintf("  ⏱️  Tiempo total: %.1f segundos\n\n", elapsed))

toc()
cat("✅ Script 07d SLS completado\n")
