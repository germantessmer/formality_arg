# =============================================================================
# [EN] 08c_reporte_hibrido_GLM.R -- HTML report for hybrid variable (ground truth + GLM backcast) with composition analysis
# INPUTS:  rdos/datos/08_panel_formalidad_GLM*.rds, contracts from 07-08 GLM
# OUTPUTS: rdos/reportes/08c_reporte_hibrido_GLM*.html
# =============================================================================
# 🌟 08c_reporte_hibrido_GLM.R 🌟 ####
# OBJETIVO:
#    Reporte HTML de la variable híbrida GLM — combina ground truth
#    (condicion_formalidad) con backcasting GLM (umbral calibración).
...
#
# DIFERENCIAS CLAVE vs 08c LPM:
#   - Columnas: formalidad_clase_{SUFIJO}_pea/edad, formalidad_clase_cal_{SUFIJO}_pea/edad
#   - Color: COL_GLM (#1F4E79) — azul oscuro
#   - Sin mención a pred. fuera [0,1] (GLM: 0.00% por construcción)
#   - S7.2: destaca calibración nativa (H-L test)
#   - S7.3: nota adaptada para GLM (prob. en [0,1] garantizadas)
#
# INPUTS:
#    - rdos/datos/08_panel_formalidad_{SUFIJO_MODELO_GLM}.rds         (Script 08)
#    - rdos/contratos/08_contrato_backcasting_{SUFIJO_MODELO_GLM}.rds (Script 08)
#    - rdos/datos/02_panel_con_taxonomia.rds             (Script 02)
#
# OUTPUTS:
#    - rdos/reportes/08c_reporte_hibrido_{SUFIJO_MODELO_GLM}.html
#    - rdos/reportes/08c_notas_paper_hibrido_{SUFIJO_MODELO_GLM}.txt
#    - rdos/figuras/08c_reporte_hibrido_GLM/  (9 PDFs)
#
...
tic("Script 08c - Reporte Variable Híbrida GLM")
start_time <- Sys.time()
cat("===================================================================\n")
cat("SCRIPT 08c - REPORTE VARIABLE HÍBRIDA GLM\n")
...
PATH_08C_HTML <- PATH_08C_HTML_GLM
PATH_TXT_08C  <- file.path(DIR_REPORTES,
                            paste0("08c_notas_paper_hibrido_", SUFIJO_MODELO_GLM, ".txt"))
...
dir.create(DIR_FIGURAS_08C_GLM, showWarnings = FALSE, recursive = TRUE)
...
panel <- panel %>%
  mutate(
    ...
    # Backcasting GLM (umbral calibración) para el resto
...
kpi_df <- tibble(
  Indicador = c(
    ...
    "N obs. backcasting GLM (formalidad híbrida)",
    "% ground truth en variable híbrida",
    "% backcasting GLM en variable híbrida",
    ...
    "Umbral Youden",
    "Umbral Calibración",
    "Calibración nativa (H-L test 07b)"
  ),
  Valor = c(
    ...
    paste0("H-L test (07b): p=",
           sprintf("%.3f", c08$hl_pvalue_07b %||% NA_real_),
...
comp_exec <- tibble(
  Fuente      = c("Ground truth (condicion_formalidad)",
                  paste0("Backcasting GLM (umbral ", sprintf("%.3g", UMBRAL_CAL), ")"),
                  "Total"),
...
# Colores semánticos para el reporte híbrido GLM
# HC documentado: COL_FORMAL y COL_INFORMAL representan concepto formalidad,
# no un modelo específico — coherencia con el modelo GLM del reporte.
...
COL_BACK_GLM <- COL_GLM             # azul oscuro — backcasting GLM
...
g_s1a <- ggplot(s1a, aes(x = periodo_id)) +
  ...
  tr_labs(title    = "S1a: Tasa formal observada vs híbrida GLM — Ocupados",
          subtitle = "4 trimestres de entrenamiento",
          x = NULL, y = "Tasa formal (%)", fill = NULL) +
  theme_paper() +
  theme(legend.position = "bottom")

guardar_figura(g_s1a, DIR_FIGURAS_08C_GLM, "s1a", 1)
...
g_s1b <- ggplot(s1b, aes(x = periodo_id)) +
  ...
  geom_col(aes(y = tasa_hibrida_pea, fill = tr("Híbrida PEA")),
           width = 0.35, position = position_nudge(x =  0.2)) +
  scale_fill_manual(values = setNames(
    c(COL_GT, COL_BACK_GLM), tr(c("Observada", "Híbrida PEA"))
  )) +
  tr_labs(title    = "S1b: Tasa formal observada vs híbrida GLM — PEA completa",
          subtitle = "Incluye desocupados potenciales",
          x = NULL, y = "Tasa formal (%)", fill = NULL) +
  theme_paper() +
  theme(legend.position = "bottom")

guardar_figura(g_s1b, DIR_FIGURAS_08C_GLM, "s1b", 2)
...
g_s2a <- ggplot(s2a_long, aes(x = periodo_id, y = Pct, fill = Fuente)) +
  ...
  scale_fill_manual(values = setNames(
    c(COL_GT, COL_BACK_GLM), tr(c("Ground truth", "Backcasting"))
  )) +
  tr_labs(title    = "S2a: Composición de la variable híbrida GLM por trimestre",
...
guardar_figura(g_s2a, DIR_FIGURAS_08C_GLM, "s2a_comp", 3)

# S2b: serie tasa formal por fuente + global
s2b <- s2b %>% mutate(!!COL_FUENTE_PEA := tr(.data[[COL_FUENTE_PEA]]))
...
g_s2b <- ggplot() +
  ...
  tr_labs(title    = "S2b: Tasa formal por fuente de la variable híbrida GLM",
...
guardar_figura(g_s2b, DIR_FIGURAS_08C_GLM, "s2b_serie", 4)
...
guardar_figura(g_s3a_comp, DIR_FIGURAS_08C_GLM, "s3a_comp", 5)
...
g_s3b <- ggplot(s3b_serie,
  ...
  tr_labs(title    = "S3b: Formalidad híbrida GLM — ocupados vs desocupados potenciales",
          x = NULL, y = "Tasa formal (%)", color = NULL) +
  theme_paper() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7),
        legend.position = "bottom")

guardar_figura(g_s3b, DIR_FIGURAS_08C_GLM, "s3b", 6)
...
g_s4_pea_activos <- ggplot(s4_long,
  ...
  tr_labs(title    = "S4: Series completas — PEA activos (3 versiones GLM)",
...
guardar_figura(g_s4_pea_activos, DIR_FIGURAS_08C_GLM, "s4_pea", 7)
...
g_s4_edad <- ggplot(s4_edad, aes(x = periodo_id, y = tasa_cal, group = 1)) +
  ...
  tr_labs(title    = "S4: Serie calibrada GLM — Universo Edad 18-60",
          subtitle = "Incluye inactivos con θ disponible",
          x = NULL, y = "Tasa formal (%)") +
  theme_paper() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7))

guardar_figura(g_s4_edad, DIR_FIGURAS_08C_GLM, "s4_edad", 8)
...
g_s5 <- ggplot() +
  ...
  tr_labs(title    = "S5: Formalidad potencial GLM — Desocupados e inactivos",
          subtitle = paste0("Umbral calibración (", UMBRAL_CAL,
                            ") | Sin sesgo de clipping"),
          x = NULL, y = "Tasa formal estimada (%)", color = NULL) +
  theme_paper() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7),
        legend.position = "bottom")

guardar_figura(g_s5, DIR_FIGURAS_08C_GLM, "s5_potencial", 9)

cat("   [OK] Graficos pre-calculados: 9 | PDFs guardados en DIR_FIGURAS_08C_GLM\n\n")
...
notas_txt <- c(
  ...
  "",
  "## CALIBRACIÓN GLM (ventaja sobre LPM)",
  paste0("Calibración nativa (H-L test 07b): p=",
         sprintf("%.3f", c08$hl_pvalue_07b %||% NA_real_)),
  "Probabilidades en [0,1]: 100% (invariante GLM binomial)",
  paste0("Delta max tasa (calibración, backcasting): ",
         sprintf("%.2f", c08$metricas_calibracion$delta_max_pp), " pp"),
  paste0("AUC-ROC test 07b: ", sprintf("%.4f", c08$auc_roc_test_07b)),
  "",
  "## N UNIVERSOS",
  ...
  "La variable híbrida GLM combina el ground truth observable (asalariados con registro",
  "jubilatorio) con estimaciones backcasting GLM para el resto del universo PEA.",
  "Ventaja del GLM sobre LPM: probabilidades en [0,1] garantizadas (sin clipping),",
  paste0("calibración nativa verificada (H-L p=",
         sprintf("%.3f", c08$hl_pvalue_07b %||% NA_real_), "), mejor tratamiento de desocupados."),
  paste0("Con umbral Cal. (", UMBRAL_CAL, "): delta max = ",
         c08$metricas_calibracion$delta_max_pp, " pp en ventana training."),
  "",
  "## FIGURAS EXPORTADAS",
  paste0("Directorio: rdos/figuras/", basename(DIR_FIGURAS_08C_GLM), "/"),
...
writeLines(c(
  '---',
  'title: "Reporte Variable Híbrida — GLM Backcasting Formalidad"',
  paste0('subtitle: "EPH Argentina ', ANIO_INI, 'T', TRIM_INI, '\u2013', ANIO_FIN, 'T',
         TRIM_FIN, ' | Modelo ', SUFIJO_MODELO_GLM, ' | Umbral Cal. ', UMBRAL_CAL, '"'),
...
writeLines(c(
  ...
  'h1 { color: #2c3e50; border-bottom: 2px solid #1F4E79; }',
  'h2 { color: #1F4E79; }',
  'h3 { color: #555; }',
  ...
  '           margin: 10px 0; border-radius: 3px; font-size: 13px; }',
  '.hibrida { background: #eaf0f9; border-left: 4px solid #1F4E79; padding: 8px 12px;',
  '            margin: 10px 0; border-radius: 3px; font-size: 13px; }',
  '</style>',
  ''
), con)
...
writeLines(c(
  ...
  '',
  '<div class="hibrida">',
  '<strong>Variable híbrida GLM:</strong> combina el ground truth administrativo',
  '(<code>condicion_formalidad</code>) para asalariados cubiertos por la metodología',
  sprintf('EPH tradicional, con el backcasting GLM (umbral de calibración %.3g) para', UMBRAL_CAL),
  ...
  'sobre el universo PEA completo. <strong>Ventaja GLM:</strong> probabilidades en [0,1]',
  sprintf('garantizadas (sin clipping), calibración nativa verificada (H-L p=%.3f).',
          c08$hl_pvalue_07b %||% NA_real_),
  '</div>',
  ''
), con)
...
writeLines(c(
  ...
  '  row_spec(c(5,6,7,8), background = "#eaf0f9") |>',
  '  row_spec(16, bold = TRUE, background = "#d6eaf8")',
  '```',
  ''
), con)
...
writeLines(c(
  ...
  '  caption = "Composición de la variable híbrida GLM — universo PEA activos + desocupados") |>',
...
writeLines(c(
  ...
  '  col.names = c("Trimestre","N obs.","Tasa obs.","Tasa híbrida GLM","Delta"),',
  '  caption = "S1a: Validación ocupados — variable híbrida GLM vs observada") |>',
  '  kable_styling(bootstrap_options = c("striped","hover","condensed"),',
  '                full_width = FALSE, font_size = 12)',
  '```',
  ''
...
writeLines(c(
  ...
  '                "Tasa híbrida GLM PEA","N PEA total","Delta PEA"),',
  '  caption = "S1b: Validación PEA — variable híbrida GLM vs observada") |>',
  '  kable_styling(bootstrap_options = c("striped","hover","condensed"),',
  '                full_width = FALSE, font_size = 12)',
  '```',
  ''
...
writeLines(c(
  '## S1.4 S1d: Métricas de clasificación (variable híbrida GLM)',
  '',
  '```{r s1d-tabla}',
  'knitr::kable(s1d,',
  '  col.names = c("Métrica","Valor"),',
  '  caption = "S1d: Métricas de clasificación — variable híbrida GLM vs ground truth") |>',
  '  kable_styling(bootstrap_options = c("striped","hover","condensed"),',
  '                full_width = FALSE, font_size = 12)',
  '```',
  ''
...
writeLines(c(
  ...
  '  caption = "S2a: Composición GT vs Backcasting GLM por trimestre") |>',
  '  kable_styling(bootstrap_options = c("striped","hover","condensed"),',
...
writeLines(c(
  ...
  '  caption = "S3a: Tasa formal híbrida GLM por categoría ocupacional (global)") |>',
  '  kable_styling(bootstrap_options = c("striped","hover","condensed"),',
  '                full_width = FALSE, font_size = 12)',
  '```',
  ''
...
writeLines(c(
  ...
  '  caption = "S3b: Tasa formal GLM — desocupados potenciales por categoría ocupacional") |>',
  '  kable_styling(bootstrap_options = c("striped","hover","condensed"),',
  '                full_width = FALSE, font_size = 12)',
  '```',
  ''
...
writeLines(c(
  ...
  '  caption = "S4.1: Series completas GLM — PEA activos") |>',
  '  kable_styling(bootstrap_options = c("striped","hover","condensed"),',
  '                full_width = FALSE, font_size = 11)',
  '```',
  ''
...
# ── S7 ───────────────────────────────────────────────────────────────────────
# (LN4) No se menciona "predicciones fuera de [0,1]" en el GLM.
# S7.3 adaptada para GLM: calibración nativa en lugar de clipping.
writeLines(c(
  '# S7. Notas metodológicas',
  '',
  ...
  '',
  '<div class="hibrida">',
  '<strong>¿Qué es la variable híbrida GLM?</strong>',
  ...
  paste0('  <li><strong>Backcasting GLM</strong> (<code>', COL_CLASE_CAL_PEA, '</code>,'),
  ...
  '  El modelo logístico binomial garantiza probabilidades en [0,1] sin necesidad de clipping.</li>',
  '</ol>',
  '</div>',
  ''
), con)
...
writeLines(c(
  ...
  paste0('- **Calibración nativa GLM:** H-L test (07b): p=',
         sprintf('%.3f', c08$hl_pvalue_07b %||% NA_real_),
         '. Las probabilidades predichas corresponden a frecuencias observadas.'),
  ''
), con)
...
writeLines(c(
  ...
  '',
  '<div class="nota">',
  'Las predicciones del modelo GLM para períodos anteriores a 2024T4 son',
  ...
  'fue verificada en 07c_lasso_tiempo_GLM.R. A diferencia del LPM, el GLM binomial',
  'garantiza probabilidades en [0,1] para todos los perfiles de predicción,',
  'incluyendo desocupados con theta_A extremo (elimina el sesgo de clipping del LPM).',
  '</div>',
  ''
), con)
...
writeLines(c(
  ...
  '<div class="hibrida">',
  paste0(
    'La **variable híbrida GLM** integra ground truth administrativo (',
    pct_gt, '% de las observaciones PEA) con backcasting GLM (', pct_back, '%), ',
  ...
  '',
  paste0(
    'La variable híbrida GLM es el input recomendado para los análisis del paper que ',
    'requieran cobertura PEA + máxima precisión para el segmento asalariado. ',
    'Ventaja sobre la versión LPM: probabilidades en [0,1] garantizadas (sin clipping), ',
    paste0('calibración nativa verificada (H-L p=',
           sprintf('%.3f', c08$hl_pvalue_07b %||% NA_real_), '). '),
  ...
  '',
  '---',
  paste0('*Script: 08c_reporte_hibrido_GLM.R | Modelo: ', SUFIJO_MODELO_GLM,
...
n_pdfs <- length(list.files(DIR_FIGURAS_08C_GLM, pattern = "\\.pdf$"))
...
cat(sprintf("  PDFs: %d/9 en %s\n", n_pdfs, basename(DIR_FIGURAS_08C_GLM)))
cat("===================================================================\n")
