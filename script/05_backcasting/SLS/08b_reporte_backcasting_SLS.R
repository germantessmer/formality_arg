# =============================================================================
# [EN] 08b_reporte_backcasting_SLS.R -- Funnel-structured HTML report: backcasting validation, historical coherence, coverage (SLS)
# INPUTS:  rdos/datos/08_panel_formalidad_SLS*.rds, contracts from 07-08 SLS
# OUTPUTS: rdos/reportes/08b_reporte_backcasting_SLS*.html
# =============================================================================
# 🌟 08b_reporte_backcasting_SLS.R 🌟 ####
# OBJETIVO:
#    Reporte HTML del backcasting SLS — estructura de embudo de comparabilidad.
...
#         [LN4] Seccion especial: predicciones fuera de [0,1] — ACTIVA en SLS
#              (a diferencia de GLM, donde esta seccion no aplica)
#
# DIFERENCIAS CLAVE vs 08b LPM:
#    - Colores: COL_SLS / COL_SLS_CLARO (violeta oscuro / violeta claro)
#    - S7: seccion [LN4] ACTIVA — pred fuera [0,1] presente (SLS es lineal)
#    - KPI tabla: "N coeficientes (lm)" en lugar de "Lambda (1se)"
#    - S7 AUC: referencia es c07a$auc_test (LASSO SLS) — sin HC
#
# INPUTS:
...
#    - rdos/contratos/07a_contrato_lasso_{SUFIJO_MODELO_SLS}.rds      (Script 07a)
#    - rdos/contratos/07b_contrato_postlasso_{SUFIJO_MODELO_SLS}.rds  (Script 07b)
#    - rdos/datos/02_panel_con_taxonomia.rds             (Script 02)
#
# OUTPUTS:
#    - rdos/reportes/08b_reporte_backcasting_{SUFIJO_MODELO_SLS}.html
#    - rdos/reportes/08b_notas_paper_{SUFIJO_MODELO_SLS}.txt
#    - rdos/figuras/08b_reporte_backcasting_SLS/  (10 PDFs)
#
...
tic("Script 08b - Reporte Backcasting SLS")
start_time <- Sys.time()
cat("===================================================================\n")
cat("SCRIPT 08b - REPORTE BACKCASTING SLS (embudo de comparabilidad)\n")
cat("[LN4] Seccion pred fuera [0,1] ACTIVA (estructura SLS)\n")
...
PATH_08B_HTML <- PATH_08B_HTML_SLS
PATH_TXT_08B  <- file.path(DIR_REPORTES, paste0("08b_notas_paper_", SUFIJO_MODELO_SLS, ".txt"))
...
hard_stop(file.exists(PATH_07_CONTRATO_SLS), paste0("No existe 07a_contrato_lasso_", SUFIJO_MODELO_SLS, ".rds"))
hard_stop(file.exists(PATH_07B_CONTRATO_SLS),paste0("No existe 07b_contrato_postlasso_", SUFIJO_MODELO_SLS, ".rds"))
dir.create(DIR_REPORTES,        showWarnings = FALSE, recursive = TRUE)
dir.create(DIR_FIGURAS_08B_SLS, showWarnings = FALSE, recursive = TRUE)

c08  <- readRDS(PATH_08_CONTRATO_SLS)
c07a <- readRDS(PATH_07_CONTRATO_SLS)
...
hard_stop(!is.na(UMBRAL_YOUDEN) && UMBRAL_YOUDEN > 0 && UMBRAL_YOUDEN < 1,
          paste("[08b SLS] UMBRAL_YOUDEN invalido desde c08:", UMBRAL_YOUDEN))
hard_stop(!is.na(UMBRAL_CAL) && UMBRAL_CAL > 0 && UMBRAL_CAL < 1,
          paste("[08b SLS] UMBRAL_CAL invalido desde c08:", UMBRAL_CAL))
...
# c07b$metricas_clf es 1x8 con el umbral Youden del post-LASSO
...
cat(sprintf("   Metricas S1d OK: Acc Youden=%.2f%% | Acc Cal=%.2f%%\n",
            acc_youden, acc_cal))

# pct_fuera_01 en Observados backcasting (calculado desde panel — no en c08)
pct_fuera_01_obs_back <- panel %>%
  filter(tipo_estimacion_pea == "Observado", !is.na(.data[[COL_PROB_PEA]])) %>%
  summarise(pct = mean(.data[[COL_PROB_PEA]] < 0 | .data[[COL_PROB_PEA]] > 1) * 100) %>%
  pull(pct) %>% round(1)
...
pct_fuera_01_test <- c07a$pct_pred_fuera_01
...
writeLines('---
title: "Backcasting SLS -- Reporte Capa 5"
...
COL_FORMAL   <- COL_SLS          # violeta — formal = SLS (coherencia)
...
# Series temporales S4: mismo color COL_SLS, diferencia por linetype + linewidth
...
LBL_BACK_CAL  <- paste0("Backcasting SLS (", LBL_CAL, ")")
...
kpi <- tibble(
  Indicador = c(
    ...
    "N union (PEA U Edad 18-60)","Modelo base","N coeficientes (lm)",
  ...
  Valor = c(
    ...
    c08$modelo_origen, sprintf("%d predictores", c08$n_coefs_lm - 1L),
...
> **S7** documenta la propiedad estructural del SLS: acotamiento garantizado en
> entrenamiento (kappa), no fuera de muestra.
...
cols_s1a <- setNames(c(PAL_DESCRIPTIVO[2], COL_SLS),
...
guardar_figura(p_s1a, DIR_FIGURAS_08B_SLS, "s1a", 1)
p_s1a
```
...
comp_s1 %>%
  ...
  kable(format="html", align=c("l","r","r","r","r","r"),
        caption=paste0("S1a: Tasa observada vs. estimacion sobre ocupados (",
                       LBL_CAL, ")")) %>%
  kable_styling(bootstrap_options=c("striped","hover","condensed"),
                full_width=FALSE) %>%
...
> Los desocupados tienen tasa formal baja (~7%), por lo que al incluirlos la tasa
> agregada baja y el delta respecto a la observada sube respecto a S1a.

```{r s1b_graf, fig.height=3.7}
s1b_long <- bind_rows(
...
cols_s1b <- setNames(c(PAL_DESCRIPTIVO[2], COL_SLS, COL_SLS_CLARO),
...
p_s1b <- ggplot(s1b_long, aes(x=periodo_id, y=tasa, color=serie, group=serie, linetype=serie)) +
  ...
  scale_color_manual(values=cols_s1b, name=NULL) +
  scale_linetype_manual(
    values=setNames(c("solid","dashed","dotted"),
                    tr(c("Observada (EPH)", "Estimada ocupados", "Estimada PEA"))), name=NULL) +
  scale_y_continuous(labels=percent_format(accuracy=0.1)) +
  ...
  theme_paper() +
  theme(axis.text.x=element_text(angle=20, hjust=1), legend.position="bottom")
guardar_figura(p_s1b, DIR_FIGURAS_08B_SLS, "s1b", 2)
p_s1b
```
...
p_s1c <- ggplot(delta_df, aes(x=periodo_id, y=pp, fill=componente)) +
  ...
  scale_fill_manual(
    values=setNames(c(COL_SLS, PAL_DESCRIPTIVO[3]),
                    tr(c("Delta ocupados vs obs.", "Contribucion desocupados"))), name=NULL) +
...
guardar_figura(p_s1c, DIR_FIGURAS_08B_SLS, "s1c", 3)
p_s1c
```
...
> Umbral calibracion (`r LBL_CAL`).

## Serie backcasting vs. metodologia anterior

```{r s2_serie, fig.height=3.7}
s2_long <- bind_rows(
  serie_asal_vieja %>%
...
lbl_back_cal_loc <- LBL_BACK_CAL
lbl_met_ant      <- tr("Metodologia anterior")
cols_s2 <- setNames(
  c(COL_FORMAL, COL_SLS, COL_INFORMAL, COL_SLS_CLARO),
  c(paste0(tr("Formal"), " -- ", lbl_met_ant),
    paste0(tr("Formal"), " -- ", lbl_back_cal_loc),
    paste0(tr("Informal"), " -- ", lbl_met_ant),
    paste0(tr("Informal"), " -- ", lbl_back_cal_loc))
)
ltys_s2 <- setNames(
...
p_s2 <- ggplot(s2_long,
  ...
  annotate("rect",
           ...
           ymin=-Inf, ymax=Inf, fill="#9b59b6", alpha=0.06) +
  annotate("rect",
           xmin=idx_obs_s2-0.5, xmax=n_s2+0.5,
           ymin=-Inf, ymax=Inf, fill="#f39c12", alpha=0.08) +
  geom_line(linewidth=0.9) + geom_point(size=1.6) +
  scale_color_manual(values=cols_s2, name=NULL) +
  scale_linetype_manual(values=ltys_s2, guide="none") +
  ...
  tr_labs(title="S2: Backcasting asalariados vs. metodologia anterior",
       subtitle="Zona naranja = observados | Zona violeta = trimestres pandemia (COVID)",
       x=NULL, y="Tasa") +
  theme_paper() +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=7),
        legend.position="bottom", legend.text=element_text(size=7))
guardar_figura(p_s2, DIR_FIGURAS_08B_SLS, "s2", 4)
p_s2
```
...
tab_cat_global %>%
  ...
  kable(format="html", align=c("l","r","r","r","r"),
        caption=paste0("S3a: Formalidad por categoria -- solo ocupados (",
                       LBL_CAL, ")")) %>%
  kable_styling(bootstrap_options=c("striped","hover","condensed"),
                full_width=FALSE) %>%
...
guardar_figura(p_s3a_serie, DIR_FIGURAS_08B_SLS, "s3a_serie", 5)
p_s3a_serie
```

```{r s3a_comp, fig.height=3.7}
colores_cond <- setNames(c(COL_FORMAL, COL_INFORMAL), tr(c("Formal","Informal")))
p_s3a_comp <- comp_cat_trim %>%
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1, size=6),
        ...
        legend.position="bottom")
guardar_figura(p_s3a_comp, DIR_FIGURAS_08B_SLS, "s3a_comp", 6)
p_s3a_comp
```
...
p_s3b <- ggplot(serie_comp_cat,
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1, size=6),
        strip.text=element_text(face="bold"),
        legend.position="bottom")
guardar_figura(p_s3b, DIR_FIGURAS_08B_SLS, "s3b", 7)
p_s3b
```
...
p_s4_youden <- ggplot(series_youden,
       aes(x=periodo_id, y=tasa, color=serie, linetype=serie,
           linewidth=serie, shape=serie, group=serie)) +
  annotate("rect", xmin=idx_ini_obs-0.5, xmax=n_trim+0.5,
           ymin=-Inf, ymax=Inf, fill="#f39c12", alpha=0.08) +
  geom_line() + geom_point(size=1.6) +
  scale_color_manual(
    values=setNames(rep(COL_SLS, 3), tr(c("Ocupados","PEA","Edad 18-60"))),
    name=tr("Universo")) +
  scale_linetype_manual(values=linetypes_series, name=tr("Universo")) +
  scale_linewidth_manual(values=linewidths_series, name=tr("Universo")) +
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1, size=7),
        legend.position="bottom")
guardar_figura(p_s4_youden, DIR_FIGURAS_08B_SLS, "s4_youden", 8)
p_s4_youden
```

## `r LBL_CAL`

```{r s4_cal, fig.height=3.7}
...
p_s4_cal <- ggplot(series_cal,
  ...
  scale_y_continuous(labels=percent_format(accuracy=0.1), limits=c(0,NA)) +
  scale_x_discrete(breaks=trim_ids[seq(1,n_trim,by=4)]) +
  tr_labs(title=paste0("S4: Series de formalidad -- ", LBL_CAL),
       subtitle="Zona sombreada = 4 trimestres con formalidad observada",
       x=NULL, y="Tasa de formalidad") +
  theme_paper() +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=7),
        legend.position="bottom")
guardar_figura(p_s4_cal, DIR_FIGURAS_08B_SLS, "s4_cal", 9)
p_s4_cal
```
...
tab_series %>%
  ...
  kable(format="html", align=c("l","r","r","r","r","r","r","r","r"),
        caption=paste0("S4: Comparacion de universos -- Umbral calibracion (",
                       LBL_CAL, ")")) %>%
  kable_styling(bootstrap_options=c("striped","hover","condensed"),
...
# Ver c08$nota_desocupados para contexto.
tibble(
  Metrica=c("N elegible","Tasa formal (Youden)","Tasa formal (Calibracion)",
            "Pred. fuera [0,1]","Nota estructural"),
  `Universo PEA`=c(
    fmt_n(n_desoc_pea),
    ...
    paste0(round(c08$universo_pea$pct_pred_fuera_01,1),
           "% (vs. ", sprintf("%.1f%%", pct_fuera_01_test)," en test SLS)"),
    "SLS no garantiza acotamiento OOS. Clipping pmax/pmin aplicado."
  )
) %>%
  kable(format="html", align=c("l","r"),
        caption="Desocupados: formalidad potencial (SLS)") %>%
  kable_styling(bootstrap_options=c("striped","hover","condensed"),
                full_width=FALSE) %>%
  row_spec(4, background="#fdebd0") %>%
  row_spec(5, background="#f8f9fa")
```

> `r c08$nota_desocupados`
>
> **Advertencia:** alta proporcion de predicciones fuera de [0,1] en desocupados.
> Extrapola fuera del soporte del entrenamiento (kappa). El GLM es mas apropiado
> para este subgrupo. Resultados con cautela.
...
p_s6 <- cob_pea %>%
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1, size=7),
        legend.position="bottom")
guardar_figura(p_s6, DIR_FIGURAS_08B_SLS, "s6_cob", 10)
p_s6
```
...
## [LN4] Predicciones fuera de [0,1] -- propiedad estructural del SLS

> **Esta seccion es especifica del SLS.** El GLM (binomial) garantiza predicciones en [0,1]
> por construccion. El LPM es lineal y puede extrapolar fuera, pero en menor medida.
> El SLS (Sequential Least Squares, Horrace & Oaxaca 2003) garantiza acotamiento en
> el subconjunto de entrenamiento kappa, pero NO fuera de muestra.

```{r s7_pred}
tibble(
  Contexto=c(
    paste0("kappa (train SLS",
           if (!is.null(c08$n_kappa_train)) paste0(", ", fmt_n(c08$n_kappa_train), " obs") else "",
           ") -- acotado por construccion"),
    paste0("Test set 07a (SLS, ", fmt_n(c07a$n_test), " obs) -- benchmark OOS"),
    paste0("Observados backcasting (", fmt_n(sum(comp_trim$n)), " obs)"),
    paste0("PEA completa -- union (", fmt_n(c08$n_union), " obs)")
  ),
  `% fuera [0,1]`=c(
    "0.00%",
  ...
  Nota=c(
    "Garantia del algoritmo SLS",
    "c07a$pct_pred_fuera_01 (benchmark HANDOFF)",
    "Referencia correcta para el paper (calculado desde panel)",
    "Incluye desocupados y backcasting historico"
  )
) %>%
  kable(format="html", align=c("l","r","l"),
        caption="[LN4] Predicciones fuera de [0,1] en distintos contextos (SLS)") %>%
  kable_styling(bootstrap_options=c("striped","hover","condensed"),
                full_width=TRUE) %>%
...
> **Interpretacion:** el incremento de 0% (kappa) al valor OOS (test) y al backcasting
> completo no es degradacion del modelo. Refleja que el panel historico incluye perfiles
> de theta_A fuera del soporte del subconjunto kappa. Los desocupados son el driver
> principal (theta_A alto + beta negativo de theta_A). Clipping pmax(0, pmin(1, pred))
> aplicado en todas las clasificaciones. Ref: Horrace & Oaxaca (2003).

## AUC backcasting vs. AUC test 07a

AUC backcasting (**`r sprintf("%.4f", c08$auc_global_train)`**) < test SLS (**`r sprintf("%.4f", auc_test_c07a)`**).
No es degradacion: el AUC test se calcula sobre el split 80/20 dentro de los 4 trimestres
de entrenamiento (ponderado). El AUC backcasting se calcula sobre los
`r fmt_n(sum(comp_trim$n))` Observados del panel completo.
...
tibble(
  ...
  Limitacion=c("Hold-out no disponible","Desocupados -- extrapolacion masiva",
               "Inactivos -- imputacion laboral",
               "Pred. fuera [0,1] (SLS -- sin garantia OOS)",
               "Clustering EPH"),
  Detalle=c(
    "Formalidad observada solo en 2024T4-2025T3. Validacion temporal restringida al training.",
    paste0("SLS: pred. fuera [0,1] en desocupados structuralmente alta (",
           round(c08$universo_pea$pct_pred_fuera_01, 1),"%). ",
           "Causa: theta_A de desocupados fuera del soporte de kappa. ",
           "GLM mas apropiado para este subgrupo."),
    "Variables laborales imputadas con moda del training. Tasa refleja theta+educacion+demografia.",
    paste0("SLS garantiza [0,1] solo en kappa. OOS: clipping aplicado. ",
           "Backcasting completo: ", round(c08$pct_pred_fuera_01_union,1), "% fuera. ",
  ...
  kable(format="html", align=c("c","l","l"),
        caption="Limitaciones metodologicas del backcasting SLS") %>%
  kable_styling(bootstrap_options=c("striped","hover","condensed"),
...
El backcasting SLS aplica el modelo 07b (post-LASSO Sequential Least Squares) al panel
completo 2016T4-2025T3 (`r fmt_n(c08$n_union)` obs elegibles). El reporte construye
confianza en cuatro escalones:
...
(`r round(mean(comp_s1$pct_desoc_pea),1)`% del universo PEA en training, tasa formal ~7%).
...
**[LN4]** Propiedad SLS: acotamiento garantizado en kappa (0%), esperado ~`r sprintf("%.1f%%", pct_fuera_01_test)` OOS
(test), observado `r sprintf("%.1f%%", pct_fuera_01_obs_back)` en Observados y
`r round(c08$pct_pred_fuera_01_union,1)`% en backcasting completo.
Clipping pmax/pmin aplicado en todas las clasificaciones.

**Siguiente paso:** Script 08c -- Reporte variable hibrida (SLS).
', con)
...
LBL_CAL_TXT <- sprintf("Cal.%.3g", UMBRAL_CAL)  # para uso fuera del chunk Rmd

txt_lines <- c(
  "# =================================================================",
  "# NOTAS PARA EL PAPER -- Backcasting SLS",
  paste0("# Generado: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
  "# Script:   08b_reporte_backcasting_SLS.R (Capa 5)",
  "# =================================================================",
  "",
  "## MODELO APLICADO",
  paste0("Modelo origen:       ", c08$modelo_origen),
  paste0("Tipo modelo:         ", c08$tipo_modelo),
  paste0("N coefs (lm):        ", c08$n_coefs_lm),
  paste0("Umbral Youden:       ", sprintf("%.4f", c08$umbral_youden)),
  paste0("Umbral calibracion:  ", sprintf("%.4f", c08$umbral_calibracion)),
  paste0("AUC-ROC test 07a (LASSO SLS): ", sprintf("%.4f (c07a$auc_test)", auc_test_c07a)),
  paste0("Pred fuera [0,1] test: ", sprintf("%.2f%% (c07a$pct_pred_fuera_01)", pct_fuera_01_test)),
  "",
  ...
  paste0("Pred fuera [0,1] union: ", sprintf("%.1f%%", c08$pct_pred_fuera_01_union)),
  paste0("Pred fuera [0,1] kappa: 0.00%  (garantia SLS)"),
  paste0("Pred fuera [0,1] test:  ", sprintf("%.2f%%", pct_fuera_01_test),
  ...
  "",
  paste0("## DESGLOSE CATEGORIA OCUPACIONAL (", LBL_CAL_TXT, ")"),
  {
    tab_cat_global %>%
  ...
  "",
  "## [LN4] PREDICCIONES FUERA [0,1] -- SLS",
  "kappa (train):       0.00%  (acotamiento garantizado)",
  paste0("Test SLS (07a):      ", sprintf("%.2f%%", pct_fuera_01_test),
         "  (c07a$pct_pred_fuera_01)"),
  paste0("Observados back.:    ", sprintf("%.1f%%", pct_fuera_01_obs_back),
         "  (calculado desde panel)"),
  paste0("Union PEA:           ", sprintf("%.1f%%", c08$pct_pred_fuera_01_union)),
  "Ref: Horrace & Oaxaca (2003). Clipping pmax(0,pmin(1,pred)) aplicado.",
  "",
  "## AUC",
  paste0("AUC test 07a (LASSO):    ", sprintf("%.4f", auc_test_c07a),
         " (c07a$auc_test)"),
  paste0("AUC global back.:        ", sprintf("%.4f", c08$auc_global_train)),
  "",
  "## LIMITACIONES",
  "L1: Hold-out no disponible -- formalidad solo en 2024T4-2025T3",
  paste0("L2: Desocupados -- pred fuera [0,1]: ",
         round(c08$universo_pea$pct_pred_fuera_01, 1), "%. GLM mas apropiado."),
  "L3: Inactivos -- variables laborales imputadas con moda training",
  paste0("L4: SLS: acotamiento garantizado solo en kappa. OOS: ",
         round(c08$pct_pred_fuera_01_union, 1), "%. Ref: Horrace & Oaxaca (2003)"),
  "L5: Clustering EPH corregido (foldid codusu CV + vcovCL OLS)",
  "",
  "## FIGURAS EXPORTADAS",
  paste0("Directorio: rdos/figuras/", basename(DIR_FIGURAS_08B_SLS), "/"),
  ...
  "",
  "## SIGUIENTE PASO",
  "Script 08c -- Reporte variable hibrida (SLS)"
)
...
n_pdfs <- length(list.files(DIR_FIGURAS_08B_SLS, pattern="\\.pdf$"))
